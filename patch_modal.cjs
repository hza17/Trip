...
const modalCode = `
            {showAddRoomModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl shadow-2xl max-w-md w-full space-y-4 text-xs font-bold animate-in zoom-in-95" dir="rtl">
                        ...
                        <form onSubmit={handleAddRoom} className="space-y-4 text-right">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" value={newRoomSize} onChange={(e) => setNewRoomSize(e.target.value)} placeholder="مثلا: 45" className="w-full font-mono bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 outline-none focus:border-blue-500 transition-all dark:text-white text-left" required dir="ltr" />
                                </div>
                                <div>
                                    ...
                                    }} placeholder="مثلا: 5,000,000" className="w-full font-mono bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 outline-none focus:border-blue-500 transition-all dark:text-white text-left" required dir="ltr" />
                                </div>
                            </div>
                            <div>
